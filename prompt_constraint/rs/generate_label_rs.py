...
label_to_id = {name: idx for idx, name in enumerate([
  "farmland","forest","grassland","industry",
    "parkinglot","resident","river"
])}
...
def generate_tags_for_dataset(knowledge_base):
    ...
    for idx, item in enumerate(knowledge_base):
        ...
        prompt = build_prompt(prompt_choice, best_match_description, '../rag/rs/label.json')
...
if __name__ == "__main__":
    with open("../rag/rs/knowledge_base_clip_rs.json", "r", encoding="utf-8") as f:
        knowledge_base = json.load(f)
    tagged_knowledge_base = generate_tags_for_dataset(knowledge_base)
    os.makedirs("../result/no_data_example/rs/first_tag", exist_ok=True)
    with open("../result/no_data_example/rs/first_tag/first_tag_qwen_rs.json", "w", encoding="utf-8") as f:
