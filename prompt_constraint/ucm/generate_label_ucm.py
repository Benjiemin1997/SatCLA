...
def final_prompt_template(prompt_choice,category_mapping_str,example_str,description_text):
    ...
    if prompt_choice == 'no_feature_description':
        final_prompt = f"""
                ...
                **Instructions**:
                1. Select the SINGLE single most relevant category NUMBER (0-20)
...
def generate_tags_for_dataset(knowledge_base):
    ...
    for idx, item in enumerate(knowledge_base):
        ...
        prompt = build_prompt(prompt_choice,best_match_description, '../rag/ucm/label.json')
...
if __name__ == "__main__":
    ...
    os.makedirs("../result/no_data_example/ucm/first_tag", exist_ok=True)
    with open("../result/no_data_example/ucm/first_tag/first_tag_qwen_ucm.json", "w", encoding="utf-8") as f:
