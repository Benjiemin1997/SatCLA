...
def find_inconsistent_tags(input_file, output_file):
    try:
        ...
        if inconsistent_records:
            ...
            print("All label are consistent, no inconsistent records found")
...
if __name__ == "__main__":
    first_tag_path = "../result/ucm/first_tag/first_tag_qwen_ucm.json"
    error_tag_path = "../result/ucm/error_tag/error_tags_qwen_ucm.json"
    inconsistent_records, error_stats = find_inconsistent_tags(first_tag_path, error_tag_path)
    model, preprocess = clip.load("ViT-L/14")
    model.eval()
    retag_results = "../result/ucm/second_tag/retag_results_qwen_ucm_.json"
    dataset_name = 'UCM'
    dataset = UCMercedLandDataset(error_tag_path, transform=preprocess)
    if dataset_name == 'UCM':
        dataset = UCMercedLandDataset(error_tag_path, transform=preprocess)
    elif dataset_name == 'RSI':
        dataset = RSIDataset(error_tag_path, transform=preprocess)
    elif dataset_name == 'RS':
        dataset = RS2800Dataset(error_tag_path, transform=preprocess)
    ...
    final_tag_path = "../result/ucm/final_result/final_tags_result_qwen_ucm.json"
